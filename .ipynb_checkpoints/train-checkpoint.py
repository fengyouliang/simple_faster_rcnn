...
# 现在让我们用具有最大iou的ground truth对象为每个anchor box分配（计算）位置（location  平移缩放）。
# 注意，我们将为所有有效的anchor box分配anchor locs，而不考虑其标签，稍后在计算损失时，我们可以使用简单的过滤器删除它们。
## my 利用GT（对应最大IOU的）和anchor来计算loc
...
# 这里的roi是根据objectness_score排序取前一定数量的roi ← (anchors + locs)，并不保证IOU，NMS去除的是重叠预测，也没有去除IOU较低框  （存疑，有待考证）
sample_roi, keep_index, gt_assignment, roi_labels = utils.get_propose_target(roi, bbox, labels, n_sample=128, pos_ratio=0.25, pos_iou_thresh=0.5, neg_iou_thresh_hi=0.5, neg_iou_thresh_lo=0.0)
...
# 这里再利用 get_propose_target() 函数利用IOU分配的label（本质分配的是bbox）
# 所以换一下思路
# 利用 IOU 分配 bbox
# 对应 bbox 的 label 为 roi 的 label 
# 对应 bbox 的 坐标来对 roi 的坐标计算得到 loc = (dx, dy, dw, dh) 
# 这时就得到了 ROI 网络的 Y
# ---------------------
...
# 在此之前的坐标都是基于原图的，这里要最终预测，将其除以 16 来得到特征图上的 rois
rois = rois.long()
...
for i in range(num_rois):
   ...
   output.append(vgg.adaptive_max_pool(im)[0].data) # torch.nn.AdaptiveMaxPool2d(7, 7)
# ---------------------ROI Pooling
...
# loc 为每个类别都预测了一个坐标（存疑，有待考证），这里的置信度是值框有物体的可能性
